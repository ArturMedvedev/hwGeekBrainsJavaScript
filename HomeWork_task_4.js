...
function handleClick(clickedButtonEvent) {
    ...
    if (card.button.innerHTML == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        ...
        showMoreText(card);
    } else if (card.button.innerHTML == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function hideMoreText(card) {
    ...
    let desc = card.wrap.querySelector(".desc");
    desc.remove();
...
function showMoreText(card) {
    ...
    let descText = "lorem ipsum dolore...";
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterend', `<div class = "desc">${descText}</div>`);
